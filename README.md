...
> **v2 of this pipeline** — [v1](https://github.com/lukebarousse/dbt-duckdb-pipeline) built the foundations on DuckDB + GitHub Actions. v2 moves the same dataset to a real warehouse: Databricks, SCD2 history, incremental loads, a scheduled job.
...
| Installed package | `dbt_utils.get_column_values()` — the roles loop reads the data instead of a typed list |
...
dbt deps                                 # macro packages (dbt_utils) — dbt_packages/ isn't committed
